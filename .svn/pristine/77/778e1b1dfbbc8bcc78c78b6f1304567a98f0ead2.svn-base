...
-(void)serviceSuccessed:(NSDictionary *)keyValues pMethod:(NSString *)method {
    ...
    if ([method isEqualToString:@"GetActiveUsersJson"]) {
        ...
        if ([keyValues[@"res"] intValue] > 0) {
            ...
            [self showAlertWithTitle:CustomLocalizedString(@"tip", nil) andMessage:@"添加好友失败，请稍后再试，或者对方已经是您的好友了哦"];
        }
...
- (IBAction)cellButtonClicked:(UIButton *)sender {
    ...
    [self showAlertWithTitle:CustomLocalizedString(@"tip", nil) withDict:dict];
...
-(void)showAlertWithTitle:(NSString *)title andMessage:(NSString *)message {
    UIAlertView *alert = [[UIAlertView alloc] initWithTitle:title message:message delegate:nil cancelButtonTitle:CustomLocalizedString(@"OK", nil) otherButtonTitles:nil, nil];
    [alert show];
}
