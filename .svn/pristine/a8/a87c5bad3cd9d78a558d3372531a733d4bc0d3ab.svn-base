...
{
    [super viewDidLoad];
    PPiFlatSegmentedControl *segmented=[[PPiFlatSegmentedControl alloc] initWithFrame:CGRectMake(10, 80, 300, 30) items:@[               @{@"text":@"运动项目",@"icon":@""},@{@"text":CustomLocalizedString(@"custom", nil),@"icon":@""}]iconPosition:IconPositionRight andSelectionBlock:^(NSUInteger segmentIndex) {}];
...
//    [service getSportItems];
    ...
    if (appDelegate.languageIndex == 1) {//英文
        [service languageGetSportItems:2];
    } else {
        [service languageGetSportItems:1];
    }
...
//    if ([method isEqualToString:@"GetSportItemsJson"])
    if ([method isEqualToString:@"LanguageGetSportItemsJson"])
    {
...
//        @property (nonatomic, retain) NSString * unit;
        for (NSDictionary *dict in keyValues) {
            ...
            if (array.count > 0) {
                Sport_Items *items = array[0];
                if (items.sportId.integerValue == [dict[@"SportID"]integerValue]) {
...
//                    items.sportId = [NSNumber numberWithInt:[dict[@"SportID"] intValue]];
        ...
        if ([keyValues[@"res"] intValue] > 0 ) {
            ...
            [self showAlertWithMessage:CustomLocalizedString(@"SubSportFaile", nil) cancelButtonTitle:CustomLocalizedString(@"OK", nil)];
        }
    }
    [self getSystemSportArrayFromLocal];
...
//        [self.navigationController pushViewController:ssvc animated:YES];
    ...
    if ([_unitNameTextField.text length] < 1 || [_unitNameTextField.text isEqual:[NSNull null]]) {[self showAlertWithMessage:CustomLocalizedString(@"FillCorrectUnit", nil) cancelButtonTitle:CustomLocalizedString(@"OK", nil)];
        return ;
        
    }
    if (![self isValueFloadValue:_carlisetextField.text]) {
        [self showAlertWithMessage:CustomLocalizedString(@"UnitIntakeCalorie", nil) cancelButtonTitle:CustomLocalizedString(@"OK", nil)];
        return ;
    }
