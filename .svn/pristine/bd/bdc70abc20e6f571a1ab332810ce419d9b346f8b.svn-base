...
- (void)viewDidLoad
{
    ...
    self.paperCheckboxLabel.text = CustomLocalizedString(@"BraceletUser", nil);
    ...
    self.paperCheckboxLabel2.text = CustomLocalizedString(@"ScaleUser", nil);
...
-(void)serviceSuccessed:(NSDictionary*)keyValues pMethod:(NSString*)method
{
    ...
    //LanguageGetCoefficientsJson
//    else if ([method isEqualToString:@"GetCoefficientsJson"])
    else if ([method isEqualToString:@"LanguageGetCoefficientsJson"])
    {
...
- (IBAction)RegisterNext:(id)sender {
    if (self.paperCheckbox.isChecked==false&& self.paperCheckbox2.isChecked== false) {
        [self showSystemAlert:CustomLocalizedString(@"SelectDevice", nil)];
        return;
    }
    
    if (_itemsArray.count<3) {
        [self showSystemAlert:CustomLocalizedString(@"ChooseThreeSports", nil)];
        return;
    }
