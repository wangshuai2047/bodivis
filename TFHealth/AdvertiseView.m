...
{
    _filePath = filePath;
    NSString * newString = [filePath substringWithRange:NSMakeRange(0, [filePath length] - 4)];
    _adView.image = [UIImage imageWithContentsOfFile:newString];
}
...
- (void)pushToAd{
    ...
    if ([application respondsToSelector:@selector(openURL:options:completionHandler:)]) {
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            [application openURL:appdelegate.openUrl options:@{}
...
//            });
        ...
        BOOL success = [application openURL:appdelegate.openUrl];
//        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1.0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
//            self.hidden = YES;
//        });
    ...
    [application openURL:appdelegate.openUrl];
