...
- (void)viewDidLoad
{
    ...
    UILabel *label = [self labelAtY:y andText:[NSString stringWithFormat:@"目标%d步",targetStepCount]];
...
-(void) labelAtXLine:(float)x
{
    ...
    UIColor *bgColor = [UIColor colorWithRed:255.0 green:255.0 blue:255.0 alpha:0.7];
    ...
    UILabel *labellast = [[ UILabel alloc]initWithFrame:CGRectMake(line_x+targetStep/xDensity-30,y+3,40,20)];
    labellast.text = [NSString stringWithFormat:@"%d步",targetStep]; //这里需要注意如果用户实际行走的步数大于目标的显示情况
    ...
    labelValue.text = [NSString stringWithFormat:@"%.f步",x];
...
-(int)updateComplated:(NSString*)t
{
    UserCoreValues *lastUserCoreValues=[self getUserCoreValues];
    if (lastUserCoreValues!=nil) {
        if ([t isEqualToString:@"卡路里"]) {
            [_radialView setText:[NSString stringWithFormat:@"%d千卡",lastUserCoreValues.calorieValue.intValue/1000 ]];
        }
        else if([t isEqualToString:@"时间"])
        {
            [_radialView setText:[NSString stringWithFormat:@"%d分钟",lastUserCoreValues.timeLenght.intValue ]];
        }
        else if([t isEqualToString:@"公里"])
        {
            [_radialView setText:[NSString stringWithFormat:@"%.1f公里",lastUserCoreValues.kmCount.floatValue ]];
        }
    }
    else
    {
        if ([t isEqualToString:@"卡路里"]) {
            [_radialView setText:@"0千卡"];
        }
        else if([t isEqualToString:@"时间"])
        {
            [_radialView setText:@"0分钟"];
        }
        else if([t isEqualToString:@"公里"])
        {
            [_radialView setText:@"0公里"];
        }
    }
}
...
- (IBAction)OtaClicked:(id)sender {
    //[HUD show:YES];
    CXAlertView *alertViewMe = [[CXAlertView alloc] initWithTitle:@"询问" message:@"您确实检测并更新固件吗？" cancelButtonTitle:nil];
