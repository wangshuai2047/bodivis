...
-(void)shareClicked:(UITapGestureRecognizer *)sender
{
    ...
    AppDelegate *appdelegate =(AppDelegate*) [[UIApplication sharedApplication] delegate];
    //状态指数/基础代谢/体重/标准体重/体脂率/BMI/水份/蛋白质/肌肉/骨质
    NSString* param = [NSString stringWithFormat:@"id=%d&data=%.1f,%.1f,%.1f,%.1f,%.1f,%.1f,%.1f,%.1f,%.1f,%.1f,",appdelegate.user.userId.integerValue,share_ztzs,share_jcdx,share_tz,share_bztz,share_tzl,share_bmi,share_sf,share_dbz,share_jr,share_gz];
...
-(void)updateUIData:(VTFatScaleTestResult*)result
{
    ...
    User_Item_Info* sFatItem =[self createNewTestItem:user.userId pItemID:[TestItemID getSplanchnaPercentFat] pValue:[NSNumber numberWithFloat:result.visceralFatContent] pDate:date];
    ...
    double healthScore =[CalcGrades calcScore:muscleItem.testValue.doubleValue/standardMuslce.doubleValue fat:fatItem.testValue.doubleValue/result.weight p:proteinItem.testValue.doubleValue/result.weight m:sclerotinItem.testValue.doubleValue/result.weight g:user.sex.intValue];
    ...
    User_Item_Info* bodyAgeItem=[self createNewTestItem:user.userId pItemID:[TestItemID getBodyAge] pValue:[NSNumber numberWithDouble:[CalcGrades calcBodyAge:healthScore age:user.birthday.intValue ]] pDate:date];
