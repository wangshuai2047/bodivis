...
-(void)btnBackAction:(id)sender{
    ...
    if ([origin.titleLabel.text isEqualToString:@"选择膳食"]
        ||[origin.titleLabel.text isEqualToString:@"添加膳食"]) {
        [self setIndexText:1 txt:@"今日膳食"];
        //ssvc.superview=self;
...
-(UIView *)navigationBarView{
    ...
    if (!_navigationBarView) {
        ...
        for (int i=0; i<itemCount; i++) {
            ...
            [button setFrame:CGRectMake(i*itemWidth+(i+1)*itemMargin+10, 0, itemWidth, 44)];
