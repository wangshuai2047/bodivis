...
-(void)initChart
{
    ...
    if ([text isEqualToString:@"睡眠"]) {
        _curLabel.text=@"睡眠时长";
    }
    else
    {
        _curLabel.text=@"消耗卡路里";
    }
...
{
    PPiFlatSegmentedControl *segmented=[[PPiFlatSegmentedControl alloc] initWithFrame:CGRectMake(10,80, 300, 30) items:@[@{@"text":@"周",@"icon":@"icon-time"},@{@"text":@"月",@"icon":@"icon-calendar"},@{@"text":@"季",@"icon":@"icon-heart-empty"},@{@"text":@"年",@"icon":@"icon-cloud-download"}]iconPosition:IconPositionRight andSelectionBlock:^(NSUInteger segmentIndex) {}];
