...
//    [self.navigationController pushViewController:ssvc animated:YES];
    ...
    if ([_unitTextField.text length] < 1 || [_unitTextField.text isEqual:[NSNull null]]) {[self showAlertWithMessage:CustomLocalizedString(@"FillCorrectUnit", nil) cancelButtonTitle:CustomLocalizedString(@"OK", nil)];
        return ;
        
    }
    if ([_timeTextField.text length] < 1 || [_timeTextField.text isEqual:[NSNull null]]) {[self showAlertWithMessage:CustomLocalizedString(@"EmptyMovementTime", nil) cancelButtonTitle:CustomLocalizedString(@"OK", nil)];
        return ;
    ...
    if (![self isValueFloadValue:_timeTextField.text]) {
        [self showAlertWithMessage:@"" cancelButtonTitle:CustomLocalizedString(@"OK", nil)];
        return;
    }
...
//    @property (nonatomic, retain) NSManagedObject *sportItemValue_user_ship;
    if ([method isEqualToString:@"UploadSportInfoJson"]) {
        if ([keyValues[@"res"] intValue] == 8) {
            ...
            [self showAlertWithMessage:CustomLocalizedString(@"AddSportSuccess", nil) cancelButtonTitle:CustomLocalizedString(@"OK", nil)];
