...
//    [s getFoodPrescription:user.userId.intValue sex:sex pbf:pbf muscle:muscleValue m1:(minMuscle+maxMuscle)/2 m2:minMuscle weeksubtact:weektarget sporttarget:sporttarget foodtarget:foodtarget];
    if (appdelegate.languageIndex == 1) {//英语
        [s TempgetFoodPrescription:user.userId.intValue sex:sex pbf:pbf muscle:muscleValue m1:(minMuscle+maxMuscle)/2 m2:minMuscle weeksubtact:weektarget sporttarget:sporttarget foodtarget:foodtarget language:2];
//        [s NewDietPrescriptionJson:user.userId.intValue language:2];
    } else {
        [s TempgetFoodPrescription:user.userId.intValue sex:sex pbf:pbf muscle:muscleValue m1:(minMuscle+maxMuscle)/2 m2:minMuscle weeksubtact:weektarget sporttarget:sporttarget foodtarget:foodtarget language:1];
//        [s NewDietPrescriptionJson:user.userId.intValue language:2];
    }
...
-(void)serviceSuccessed:(NSDictionary*)keyValues pMethod:(NSString*)method
{
    ...
    {
        [self addFailedViewItem:CustomLocalizedString(@"AnaNutriFailed", nil) isTotal:false];
    }
}

-(void)serviceFailed:(NSString*) message pMethod:(NSString*)method
{
    [self addFailedViewItem:CustomLocalizedString(@"ConNetFailed", nil) isTotal:false];
}
...
//            [self addViewItemsHeader:@"早餐"];
            [self addViewItemsHeader:CustomLocalizedString(@"labelBreakfast", nil)];
            break;
        case 2:
//            [self addViewItemsHeader:@"午餐"];
            [self addViewItemsHeader:CustomLocalizedString(@"labelLunch", nil)];
            break;
        case 3:
//            [self addViewItemsHeader:@"晚餐"];
            [self addViewItemsHeader:CustomLocalizedString(@"labelDinner", nil)];
            break;
    }
    for(id obj in clist){
        int height=0;
...
-(void)addViewItemsHeader:(NSString*)headerText
{
    ...
    label.frame = CGRectMake(24, height, 55, 26);
    [self.text_view_container addSubview:label];
    
    UIView * lineView = [[UIView alloc]initWithFrame:CGRectMake(80, height+13, 205, 1)];
...
{
    if (isTotal) {
        content = [NSString stringWithFormat:@"%@%@%@",CustomLocalizedString(@"Total", nil),content,CustomLocalizedString(@"Kcalories", nil)];
    }
    else
    {
        content = [NSString stringWithFormat:@"%@%@%@",CustomLocalizedString(@"Subtotal", nil),content,CustomLocalizedString(@"Kcalories", nil)];
    }
    int height=0;
    for(UIView* view in self.text_view_container.subviews)
    {
...
-(void)addFailedViewItem:(NSString*)content isTotal:(bool)isTotal
{
    if (isTotal) {
