...
- (void)viewDidLoad
{
    ...
    if (lastUserCoreValues!=nil) {
        ...
        [_sleepTotal setText:[NSString stringWithFormat:@"%@%.1f%@   %@%.1f%@",CustomLocalizedString(@"DeepSleep", nil),lastUserCoreValues.sleepTimeCount.floatValue/60,CustomLocalizedString(@"Hour", nil),CustomLocalizedString(@"LightSleep", nil),lastUserCoreValues.lightSleepCount.floatValue/60,CustomLocalizedString(@"Hour", nil)]];
    }
    if (_curSleepCount==0) {
        _curSleepCount=0.02;
    }
    ...
    [_radialView setText:[NSString stringWithFormat:@"%.1f%@",_curSleepCount,CustomLocalizedString(@"Hour", nil)]];
    
    
    y += 110;
    UILabel *label = [self labelAtY:y andText:[NSString stringWithFormat:@"%@%.1f%@",CustomLocalizedString(@"Target", nil),targetSleepTime,CustomLocalizedString(@"Hour", nil)]];
...
-(void)updateSleepTime:(WTYGetSleepInfoModel*)object
{
    ...
    [_sleepTotal setText:[NSString stringWithFormat:@"%@%.1f%@   %@%.1f%@",CustomLocalizedString(@"DeepSleep", nil),_deepSleepCount,CustomLocalizedString(@"Hour", nil),CustomLocalizedString(@"LightSleep", nil),_lightSleepCount,CustomLocalizedString(@"Hour", nil)]];
    
    [UIView animateWithDuration:0.2 animations:^{
        [self.radialView setText:[NSString stringWithFormat:@"%.1f%@",_curSleepCount,CustomLocalizedString(@"Hour", nil)]];  //这里更新为什么不好使，不能立即显示？？？
    }];
...
{
    if (!self.onconnButton.selected) {
        CXAlertView *alertViewMe = [[CXAlertView alloc] initWithTitle:CustomLocalizedString(@"tip", nil) message:CustomLocalizedString(@"ConBraceletRadSleep", nil) cancelButtonTitle:CustomLocalizedString(@"OK", nil)];
...
-(void)goSleepView
{
    ...
    [uv setIndexText:1 txt:CustomLocalizedString(@"Sports", nil)];
...
{
    if (!self.onconnButton.selected) {
        CXAlertView *alertViewMe = [[CXAlertView alloc] initWithTitle:CustomLocalizedString(@"tip", nil) message:CustomLocalizedString(@"ConBraBeforeSleep", nil) cancelButtonTitle:CustomLocalizedString(@"OK", nil)];
        
        [alertViewMe show];
    }
    else if(self.sleepButton.selected)
    {
        CXAlertView *alertViewMe = [[CXAlertView alloc] initWithTitle:CustomLocalizedString(@"tip", nil) message:CustomLocalizedString(@"StopSleep", nil) cancelButtonTitle:nil];
        ...
        [alertViewMe addButtonWithTitle:CustomLocalizedString(@"yes", nil)
                                ...
                                handler:^(CXAlertView *alertView, CXAlertButtonItem *button) {
                                    [alertView dismiss];
                                    //isUNline=true;
                                    HUD.labelText = CustomLocalizedString(@"IsEnd", nil);
