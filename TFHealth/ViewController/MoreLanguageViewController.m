...
//    arrTable = [NSArray arrayWithObjects:@"简体中文",@"繁體中文（台灣）",@"繁體中文（香港）",@"English", nil];
    arrTable = [NSArray arrayWithObjects:@"简体中文",@"English", nil];
...
-(UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (selectPath.section == indexPath.section && selectPath.row == indexPath.row) {
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (currentPath.section == indexPath.section && currentPath.row == indexPath.row) {
        right.titleLabel.alpha = 0.5;
    }else{
