...
- (IBAction)RegisterNext:(id)sender {
    ...
    if (!(_password.text.length>=6&&_password.text.length<=12)) {
        [self showSystemAlert:CustomLocalizedString(@"PwdLengthNotCorrect", nil)];
        return;
    }
    else if(![_password.text isEqualToString:_confirmPassword.text])
    {
        [self showSystemAlert:CustomLocalizedString(@"PWDNotUnified", nil)];
        return;
    }
    else if (![predicate evaluateWithObject:_password.text])
    {
        [self showSystemAlert:CustomLocalizedString(@"DigitsOrLetters", nil)];
        return;
    }
    /*
...
-(void)serviceSuccessed:(NSDictionary*)keyValues pMethod:(NSString*)method
{
    ...
    if(keyValues.count>0 && [[keyValues allKeys] containsObject:@"res" ])
    {
        ...
        {
            [self showSystemAlert:CustomLocalizedString(@"UserNameExists", nil)];
        }
        else
        {
            [self showSystemAlert:CustomLocalizedString(@"RegistFailed", nil)];
        }
    }
    else
    {
