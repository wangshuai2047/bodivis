...
-(void) initData
{
    ...
    for (PersonalSet *p in targets) {
        ...
        NSString *sport_desc=[NSString stringWithFormat:CustomLocalizedString(@"sport_desc", nil),_st,_et,sportCount,sportSum/1000];;
        ...
        if (appdelegate.languageIndex == 1) {//英文
            food_desc=[NSString stringWithFormat:CustomLocalizedString(@"food_desc", nil),_st,_et,foodSum,foodAvg];
        } else {//中文简体
            food_desc=[NSString stringWithFormat:CustomLocalizedString(@"food_desc", nil),_st,_et,foodSum,foodAvg];
        }
...
-(NSString*)getReportTitle:(NSArray *)items stDate:(NSDate*)startDate tWeight:(float)targetWeight
{
    ...
    NSString *status = CustomLocalizedString(@"NoReach", nil);
    float curWeight=0;
    if (items.count>0) {
    ...
    if (targetWeight>=curWeight&&curWeight>0) {
        status=CustomLocalizedString(@"Reach", nil);
    }
    
    AppDelegate *appdelegate =(AppDelegate*) [[UIApplication sharedApplication] delegate];
    
    if (appdelegate.languageIndex == 1) {//英文
        report_desc= [NSString stringWithFormat:CustomLocalizedString(@"report_desc", nil),targetWeight,sDate];
    } else {//中文
        report_desc= [NSString stringWithFormat:CustomLocalizedString(@"report_desc", nil),sDate,targetWeight];
    }
    return report_desc;
}
...
-(UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([model.comp_desc isEqualToString:[NSString stringWithFormat:@"(%@)",CustomLocalizedString(@"Reach", nil)]]) {
