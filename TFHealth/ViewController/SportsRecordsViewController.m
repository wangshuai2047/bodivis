//
//  SportsRecordsViewController.m
//  TFHealth
//
//  Created by hzth hzth－mac2 on 6/5/14.
...
@interface SportsRecordsViewController ()
...
@implementation SportsRecordsViewController
...
-(void)initChart
{
    ...
    NSString* text =[uv getIndexText:1];
    if ([text isEqualToString:CustomLocalizedString(@"Sleep", nil)]) {
        _curLabel.text=CustomLocalizedString(@"SleepDuration", nil);
    }
    else
    {
        _curLabel.text=CustomLocalizedString(@"calories", nil);
    }
    
    for(UIView* view in self.chart_view.subviews)
    {
    ...
    NSMutableArray* sportValues=[NSMutableArray arrayWithCapacity:7];
    
    NSArray * sports = [UserCoreValues MR_findAllSortedBy:@"createTime" ascending:YES withPredicate:[NSPredicate predicateWithFormat:@"userId==%d AND createTime>=%@ And createTime < %@",user.userId.intValue,_startDate,_endDate] inContext:[NSManagedObjectContext MR_defaultContext]];
    ...
    for (int i=0; i<tmpLables.count; i++) {
        ...
        for (int j =0; j<sports.count; j++) {
            UserCoreValues* info = [sports objectAtIndex:j];
            float x =[info.createTime timeIntervalSinceDate:date];
            float y =[info.createTime timeIntervalSinceDate:date1];
            if (x>=0&&y<0 && info.calorieValue>0){
                if ([text isEqualToString:CustomLocalizedString(@"Sleep", nil)]) {
                    sportSum += (info.sleepTimeCount.floatValue/60);
                }
                else
                {
                    sportSum += (info.calorieValue.floatValue/1000);
                }
                sportCount++;
            }
        }
        float sportAvg =0;
        if(sportCount>0)sportAvg=sportSum/sportCount;
        [sportValues addObject: [NSNumber numberWithFloat:sportAvg]];
    ...
    [self.chart_view addSubview:view];
}
...
//    [formateymd setDateFormat:@"yy年M月d日"];
    if (appDelegate.languageIndex == 1) {//英文
        [formateymd setDateFormat:@"MM/dd/yyyy"];
    } else {
        [formateymd setDateFormat:@"yy年M月d日"];
    }
...
{
    PPiFlatSegmentedControl *segmented=[[PPiFlatSegmentedControl alloc] initWithFrame:CGRectMake(10,80, 300, 30) items:@[@{@"text":CustomLocalizedString(@"labelWeek", nil),@"icon":@"icon-time"},@{@"text":CustomLocalizedString(@"labelMonth", nil),@"icon":@"icon-calendar"},@{@"text":CustomLocalizedString(@"labelSeason", nil),@"icon":@"icon-heart-empty"},@{@"text":CustomLocalizedString(@"labelYear", nil),@"icon":@"icon-cloud-download"}]iconPosition:IconPositionRight andSelectionBlock:^(NSUInteger segmentIndex) {}];
