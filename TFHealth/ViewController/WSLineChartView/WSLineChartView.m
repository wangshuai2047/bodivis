...
- (id)initWithFrame:(CGRect)frame xTitleArray:(NSArray*)xTitleArray yValueArray:(NSArray*)yValueArray yMax:(CGFloat)yMax yMin:(CGFloat)yMin lineColor:(UIColor *)lineColor{
    ...
    if (self) {
        ...
        [self creatYAxisView];
        [self creatXAxisViewWithLineColor:lineColor];
...
- (void)creatXAxisViewWithLineColor:(UIColor*)lineColor {
    ...
    if (self.xTitleArray.count < 6) {
        self.xAxisView = [[XAxisView alloc] initWithFrame:CGRectMake(0, 0, self.frame.size.width - leftMargin, self.frame.size.height) xTitleArray:self.xTitleArray yValueArray:self.yValueArray yMax:self.yMax yMin:self.yMin];
    } else {
    ...
    self.xAxisView.lineColor = lineColor;
...
//                [SVProgressHUD showErrorWithStatus:@"已经放至最大"];
                [SVProgressHUD showErrorWithStatus:CustomLocalizedString(@"Maximum", nil)];
            }
            self.xAxisView.pointGap = self.pointGap;
