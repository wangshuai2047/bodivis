...
//                UIColor*aColor = [UIColor lightGrayColor]; //点的颜色
                UIColor*aColor = self.lineColor;
                CGContextSetFillColorWithColor(context, aColor.CGColor);//填充颜色
                CGContextAddArc(context, endPoint.x, endPoint.y, ArcRadius, 0, 2*M_PI, 0); //添加一个圆
                CGContextDrawPath(context, kCGPathFill);//绘制填充
                //添加小圆
                UIColor*littleColor = [UIColor lightTextColor];
                CGContextSetFillColorWithColor(context, littleColor.CGColor);//填充颜色
                CGContextAddArc(context, endPoint.x, endPoint.y, LittleArcRadius, 0, 2*M_PI, 0); //添加一个圆
                CGContextDrawPath(context, kCGPathFill);//绘制填充
                
                
                //画点上的文字
                NSString *str = [NSString stringWithFormat:@"%.1f", endValue.floatValue];
                // 判断是不是小数
...
//                [self drawLine:context startPoint:startPoint endPoint:endPoint lineColor:[UIColor colorWithRed:26/255.0 green:135/255.0 blue:254/255.0 alpha:1] lineWidth:2];
                [self drawLine:context startPoint:startPoint endPoint:endPoint lineColor:self.lineColor lineWidth:2];
...
//                UIColor*aColor = [UIColor lightGrayColor];//点的颜色
                UIColor*aColor = self.lineColor;
                CGContextSetFillColorWithColor(context, aColor.CGColor);//填充颜色
                CGContextAddArc(context, startPoint.x, startPoint.y, ArcRadius, 0, 2*M_PI, 0); //添加一个圆
                CGContextDrawPath(context, kCGPathFill);//绘制填充
                //添加小圆
                UIColor*littleColor = [UIColor lightTextColor];
                CGContextSetFillColorWithColor(context, littleColor.CGColor);//填充颜色
                CGContextAddArc(context, startPoint.x, startPoint.y, LittleArcRadius, 0, 2*M_PI, 0); //添加一个圆
                CGContextDrawPath(context, kCGPathFill);//绘制填充
    ...
    if(self.isLongPress)
    {
        ...
        if(nowPoint >= 0 && nowPoint < [self.yValueArray count]) {
            ...
            if ([self isPureFloat:[num floatValue]]) {
                [[NSString stringWithFormat:@"水位:%.1fm", [num floatValue]] drawAtPoint:CGPointMake(drawPoint.x, drawPoint.y+15) withAttributes:@{NSFontAttributeName:[UIFont systemFontOfSize:12],NSForegroundColorAttributeName:[UIColor whiteColor]}];
            }
            else {
...
-(void)drawYPolylineWithYArray:(NSArray*)yArray
{
