...
//    isOnclicks=1;
    ...
    {
        [self showSystemAlert:CustomLocalizedString(@"ContainSpaces", nil)];
        return;
    }
    else if (name.length > 12)
    {
        [self showSystemAlert:CustomLocalizedString(@"LongNickName", nil)];
        return;
    }
    if (age==0) {
...
//    loading.hidden = NO;
    ...
    NSString *encodedString = (NSString *)CFBridgingRelease(CFURLCreateStringByAddingPercentEscapes(kCFAllocatorDefault, (CFStringRef)name, (CFStringRef)@"!$&'()*+,-./:;=?@_~%#[]", NULL, kCFStringEncodingUTF8));
    if (isModifyMode!=1) {
        //正常模式(添加家庭成员模式)
        [s AddMembers:appdelegate.userId userIco:UIImagePNGRepresentation(self.portraitImageView.image) nick:encodedString userHeight:height userWeight:60 userSex:_sexId userAge:age userType:@"0" relationshipName:@"家人" addTime:[formate stringFromDate:currentDate]];
    }
    else
    {
        //修改模式
        [s ImproveUserData:appdelegate.user.userId.intValue userIco:UIImagePNGRepresentation(self.portraitImageView.image) nick:encodedString userAge:age userSex:_sexId userHeight:height remark:@""];
    }
}
    ...
    if(keyValues.count>0 && [[keyValues allKeys] containsObject:@"res" ])
    {
        ...
        if (res>0) {
            ...
            else
            {
                ...
                device.deviceType = [NSNumber numberWithInt:1];//默认秤用户
