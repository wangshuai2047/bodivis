...
//    loading.hidden = NO;
    ...
    [s ImproveUserData:(int)_userId userIco:imageData nick:encodedString userAge:uAge userSex:(int)_sexId userHeight:uHeight remark:@""];
}
...
-(void)serviceSuccessed:(NSDictionary*)keyValues pMethod:(NSString*)method
{
    ...
    if(keyValues.count>0 && [[keyValues allKeys] containsObject:@"res" ])
    {
        ...
        {
            [self showSystemAlert:CustomLocalizedString(@"ImpDaFaiNet", nil)];
        }
    }
    else
    {
        [self showSystemAlert:CustomLocalizedString(@"DataFaiInfo", nil)];
    }
}
