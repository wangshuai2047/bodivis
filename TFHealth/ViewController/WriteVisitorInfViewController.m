...
- (IBAction)saveFamily:(id)sender {
    ...
    [s AddMembers:appdelegate.userId userIco:nil nick:encodedString userHeight:height userWeight:60 userSex:_sexId userAge:age userType:@"1" relationshipName:@"访客" addTime:[formate stringFromDate:currentDate]];
...
{
    [self showSystemAlert:CustomLocalizedString(@"DataFailInfoAndNet", nil)];
}
...
-(void)AddUser:(NSDictionary*)keyValues
{
    if(keyValues.count>0 && [[keyValues allKeys] containsObject:@"res" ])
    {
        ...
        {
            [self showSystemAlert:CustomLocalizedString(@"AddVisitFailInfo", nil)];
        }
    }
    else
    {
